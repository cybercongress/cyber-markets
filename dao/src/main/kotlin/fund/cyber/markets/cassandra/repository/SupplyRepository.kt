...
class SupplyRepository(cassandra: Cluster) {
    ...
    private val supplyMapper by lazy { manager.mapper(Supply::class.java) }

    fun save(supply: Supply) {
        supplyMapper.save(supply)
    }

    fun saveAll(supplies: List<Supply>) {
        Flowable.fromIterable(supplies)
