...
class BitfinexMessageParserTest {
    ...
    fun testParseTrade() {
        ...
        assertEquals("43334639", trade.tradeId)
        assertEquals(bitfinex, trade.exchange)
        assertEquals(1499972199, trade.timestamp)
        assertEquals(SELL, trade.type)
        assertEquals(tokensPair, trade.tokensPair)
        assertEquals(BigDecimal("0.01293103"), trade.baseAmount)
        assertEquals(BigDecimal("0.01293103") * BigDecimal("2320"), trade.quoteAmount)
        assertEquals(BigDecimal("2320"), trade.spotPrice)
    }
}